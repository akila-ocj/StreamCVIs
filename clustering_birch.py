...
def main(input_folder, output_folder):
    ...
    for _ in range(50):  # Reduced to 50 for quicker testing; adjust as needed
        chosen_params = {
            'n_clusters': n_clusters,
            'threshold': random.choice(np.linspace(0.01, 1.5, 50)),
            'branching_factor': random.choice([25, 50, 75, 100])
        }

        model = Birch(**chosen_params)
        model.fit(X_train)
...
if __name__ == '__main__':
    import argparse
    parser = argparse.ArgumentParser(description='Train Birch and predict labels on test sets.')
